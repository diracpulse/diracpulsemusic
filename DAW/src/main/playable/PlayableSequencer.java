...
public class PlayableSequencer implements PlayableModule {
	...
	public double[] masterGetSamples(int numSamples) {
		...
		for(int index = 0; index < numSamples; index++) {
			...
			//ringpwmLFO.newSample(freqRatio);
			ring *= main;
			//ampLFO.newSample(freqRatio);
			double ringVal = ringLevel.getSample();
			returnVal[index] = (ring * ringVal + main * (1.0 - ringVal)) * (ampENV1.getSample(currentTimeInSamples) + ampENV2.getSample(currentTimeInSamples)) / 2.0;		
			double filterInput = (filterENV1.getSample(currentTimeInSamples) + filterENV2.getSample(currentTimeInSamples)) / 2.0 + filterLFO.arFilter();
